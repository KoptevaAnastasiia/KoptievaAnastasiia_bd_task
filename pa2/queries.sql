...
-- Select distinct hairdressers' names who work with massage services but are not working on a specific date.
SELECT DISTINCT e.name AS hairdresser
FROM employee e
         JOIN employee_service es ON e.employee_id = es.employee_id
WHERE es.service_id IN (SELECT id FROM service WHERE name LIKE '%Haircut')
  AND e.employee_id NOT IN (SELECT DISTINCT a.employee_id FROM appointment a WHERE DATE(a.datetime) BETWEEN '2024-03-21' AND '2024-03-27');
